## 4.1.2 GLMM (main analyses - 50km grid cells, 100km radius, 3 cells per sample, unstandardised occurrences)
...
#### Sticking with best model from main analyses ####
...
## Check assumptions
testDispersion(resbestModel, type = "DHARMa") ## Dispersion ratio is 1.0002. P-value = 0.816. Amazing!
testZeroInflation(resbestModel) ## Ratio is 1.1066.. P-value = 0.264. Better.
testOutliers(resbestModel, type = "bootstrap") ## Better!. Observed 0.0053 versus expected 0.0075. Insignificant (p = 0.7)
plot(resbestModel) ## Better!
check_collinearity(bestModel) ## VIF marginally better (bivalves = 2.93).
...
## Insignificant p-value. Actual difference between expected (-0.0013) and observed (-0.0012) is tiny. Plot looks fine.
...
resBestModel <- simulateResiduals(bestModel, re.form = NULL)
testDispersion(resBestModel, type = "DHARMa") ## Insignificant. Ratio is 0.99383.
testZeroInflation(resBestModel) ## Insignificant (p = 0.264), but a little better. Ratio is 1.1122
testOutliers(resBestModel, type = "bootstrap") ## Significant but tiny difference (Outliers = 0.0013 observed versus 0.00833 expected)
plot(resBestModel) ## KS-test failure but nothing worth worrying about - sample size issue.
check_collinearity(bestModel) ## Greatest VIF is bivalve at 2.92. Fine!
...
## Insignificant (p = 0.7585). However, inspecting the plot shows no real correlation in residuals and difference between observed (-0.00015) and expected (-0.0014) is negligible.
...
axis.labels <- axis.labels[c(5,6,1,3,4,2)]
colours <- colours[c(5,6,1,3,4,2)]
...
## Summarise - any significant terms?
