## 3.3 Estimating species richness from grid cells
...
species_noCov <- readRDS("data/final/final_100_species_noCov.Rds")
home <- getwd()
...
sp_noCov_stageCells <- unique(species_noCov$stage_cell)
sp_noCov_stages <- as.numeric(str_split_i(sp_noCov_stageCells, "_", 1))
sp_noCov_cells <- as.numeric(str_split_i(sp_noCov_stageCells, "_", 2))
sp_noCov_template <- data.frame(cbind("stage_cell" = sp_noCov_stageCells, "stage" = sp_noCov_stages, "cell" = sp_noCov_cells))

## Add empty containers to template
sp_noCov_template$bivalve <- NA
sp_noCov_template$brachiopod <- NA
sp_noCov_template$cellPalaeoLng <- extract_cell_metaData(species_noCov, target = "cellx_100km", cell = "stage_cell")
sp_noCov_template$cellPalaeoLat <- extract_cell_metaData(species_noCov, target = "celly_100km", cell = "stage_cell")
sp_noCov_template$cellAbsLat <- abs(extract_cell_metaData(species_noCov, target = "celly_100km", cell = "stage_cell"))
sp_noCov_template$PTME <- ""

## Add categorisation of PostPTME/PrePTME.
## Stages 1-48 are PrePTME, 49-92 are PostPTME
sp_noCov_template[which(sp_noCov_template$stage <49),"PTME"] <- "PrePTME"
sp_noCov_template[which(sp_noCov_template$stage >48),"PTME"] <- "PostPTME"

## Create containers for 3 analyses
sp_NC_raw <- sp_NC_CR20 <- sp_NC_CRV <- sp_noCov_template

## Clean up environment
rm(list = setdiff(ls(), c("sp_CO_raw", "sp_CO_CR20", "sp_CO_CRV", "sp_COI_raw", "sp_COI_CR20", "sp_COI_CRV", "sp_NC_raw", "sp_NC_CR20", "sp_NC_CRV", "species_covInt", "species_covPrun", "species_noCov")))
...
sample_sizes_noCov <- raw_sample_size(species_noCov)

## Match them to grid cells
sp_CO_raw$n <- NA
sp_COI_raw$n <- NA
sp_NC_raw$n <- NA
sp_CO_raw[,"n"] <- as.numeric(sample_sizes_CO[,"n"])
sp_COI_raw[,"n"] <- as.numeric(sample_sizes_COI[,"n"])
sp_NC_raw[,"n"] <- as.numeric(sample_sizes_noCov[,"n"])
...
write.csv(sp_NC_raw, file = "data/analysis_data/species_NC_raw.csv")
...
noCov_CR20 <- CR_richness(data = species_noCov, n = nOccs, n.cores = 8)

## Enter into final data tables
sp_CO_CR20[,c(4,5)] <- CO_CR20[,c(2,3)]
sp_COI_CR20[,c(4,5)] <- COI_CR20[,c(2,3)]
sp_NC_CR20[,c(4,5)] <- noCov_CR20[,c(2,3)]
...
write.csv(sp_NC_CR20, file = "data/analysis_data/species_NC_CR20.csv")
...
sp_NC_RareC <- sp_RareC[,which(RC_GCs %in% unique(species_noCov$stage_cell))]
...
sp_NC_RareC_bool <- test_RC_tail_asymptote(RCs = sp_NC_RareC, n = asymptote.occs, threshold = slope.threshold)
names(which(!sp_COI_RareC_bool))
names(which(!sp_CO_RareC_bool))
names(which(!sp_NC_RareC_bool))
...
sp_NC_n <- find_slope(sp_NC_RareC, n = 5, min.n = 20, threshold = 0.25, n.cores = 8)

## Run
CO_CRV <- CR_richness(data = species_covPrun, n = sp_CO_n, n.cores = 8)
COI_CRV <- CR_richness(data = species_covInt, n = sp_COI_n, n.cores = 8)
noCov_CRV <- CR_richness(data = species_noCov, n = sp_NC_n, n.cores = 8)

## Enter into final data tables
sp_CO_CRV[,c(4,5)] <- CO_CRV[,c(2,3)]
sp_COI_CRV[,c(4,5)] <- COI_CRV[,c(2,3)]
sp_NC_CRV[,c(4,5)] <- noCov_CRV[,c(2,3)]

## Match them to grid cells
sp_CO_CRV$n <- NA
sp_COI_CRV$n <- NA
sp_NC_CRV$n <- NA
sp_CO_CRV[,"n"] <- as.numeric(sp_CO_n)
sp_COI_CRV[,"n"] <- as.numeric(sp_COI_n)
sp_NC_CRV[,"n"] <- as.numeric(sp_NC_n)
...
write.csv(sp_NC_CRV, file = "data/analysis_data/species_NC_CRV.csv")
