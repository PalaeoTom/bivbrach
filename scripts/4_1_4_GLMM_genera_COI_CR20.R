## 4.1.4 GLMM - genera, original and interpolated, CR20
...
## Load data
raw_data <- read.csv("data/analysis_data/genera_COI_CR20.csv", header = T, row.names = 1)
...
## Result: ratio is 1.0913, significant but only just.
...
## Result: ratio of 0.61689, significant and much worse.
...
## Result: ratio of 0.55608, significant.
...
## Result: ratio of 0.55598, significant.
...
## Result: ratio of 0.58004, significant.
...
## Result: ratio of 0.52099, significant.
...
testDispersion(resBestModel, type = "DHARMa") ## Ratio of 1.0913, significant.
testZeroInflation(resBestModel) ## Ratio of 1.1961, significant.
testOutliers(resBestModel, type = "bootstrap") ## Expect freq. of 0.00516 versus observed 0.01387. Significant.
plot(resBestModel) ## All tests fail but only just.
...
testDispersion(resbestModel_sqrt, type = "DHARMa") ## Ratio of 1.1126, significant (p = 0.008). A bit better!
testZeroInflation(resbestModel_sqrt) ## Ratio of 1.2334, significant. Marginally worse.
testOutliers(resbestModel_sqrt, type = "bootstrap") ## Expect freq. of 0.0052 versus observed 0.0057. Insignificant. Better!
plot(resbestModel_sqrt) ## All tests fail but the plots themselves look fine. Could be down to sample size?
...
## Assumptions violated but almost certainly a sample size issue.
...
## p = 0.9621. However, no visible correlation from plot.
...
## Re-do best model just to make sure it has no convergence issues. Just dropped interaction term.
bestModel <- glmer(brachiopod ~ bivalve + PTME + bath + reef + lith + AbsLat + (bivalve|stage), data = modelData, family = poisson(link = "sqrt"), control = glmerControl(optimizer="bobyqa"))

## Test assumptions once again to be sure!
resBestModel <- simulateResiduals(bestModel, re.form = NULL)
testDispersion(resBestModel, type = "DHARMa") ## Ratio is 1.1097, significant.
testZeroInflation(resBestModel) ## Ratio is 1.2354, significant.
testOutliers(resBestModel, type = "bootstrap") ## Significant. Observed 0.013397 versus 0.0052 expected.
plot(resBestModel) ## All fail but visable look fine.
...
## Inspection shows no obvious correlation. P-value = 0.6488
