## 4.1.2 GLMM (main analyses - 50km grid cells, 100km radius, 2 cells per sample, standardised occurrences)
...
## Load data
raw_data <- read.csv("data/analysis_data/genera_2cell_stdOccs_Repl.csv", header = T, row.names = 1)
...
## Very overdispersed - significant ratio of 2.0577 well over 1.
...
## Now significantly underdispersed! Ratio of 0.37169
...
## Best by far. Still significant but dispersion is only 0.70216.
...
## Marginally better at 0.70439 but still significant. Sticking with nbinom1() to aid convergence
...
## No better. Dispersion ratio is 0.63015. Still significant.
...
## No better. Dispersion ratio is 0.67431. But insignificant.
...
## Sqrt significantly underdispersed (ratio = 0.024503). Sticking with link = "log" as link function.

## Check all assumptions at this point
resBestModel <- simulateResiduals(bestModel, re.form = NULL)
testDispersion(resBestModel, type = "DHARMa") ## We know this already. 0.70216
testZeroInflation(resBestModel) ## Significant with a little zero inflation. Ratio is 1.2481.
testOutliers(resBestModel, type = "bootstrap") ## Insignificant (p = 0.06). Outliers = 0.0027 observed versus 0.0071 expected
plot(resBestModel) ## Normality fine but dispersion and outlier failure. Quantiles generally fine.
check_collinearity(bestModel) ## Greatest VIF is bivalve at 2.2. All low.
...
testDispersion(resbestModel_disp, type = "DHARMa") ## Significant but better at 0.76881.
testZeroInflation(resbestModel_disp) ## Ratio is 1.1341. Significant (p=0.04). A tiny bit better.
testOutliers(resbestModel_disp, type = "bootstrap") ## Insignificant (p = 0.2). Outliers 0.0041 observed versus 0.0074 expected.
plot(resbestModel_disp) ## Still dispersion and outlier issues.
check_collinearity(bestModel_disp) ## VIFs even better (bivalve = 1.83).

resbestModel_zero <- simulateResiduals(bestModel_zero, re.form = NULL)
testDispersion(resbestModel_zero, type = "DHARMa") ## Ratio is 0.69074. Marginally worse!
testZeroInflation(resbestModel_zero) ## Ratio is 1.2342. Significant (p=0.008). Again, marginally worse.
testOutliers(resbestModel_zero, type = "bootstrap") ## Better (p=0.58) Expected (0.0067) versus observed (0.0049).
plot(resbestModel_zero) ## Still fine. Still dispersion and outlier issues.
check_collinearity(bestModel_zero) ## A better (bivalve VIF is 2.15) but still fine.

resbestModel_zeroDisp <- simulateResiduals(bestModel_zeroDisp, re.form = NULL)
testDispersion(resbestModel_zeroDisp, type = "DHARMa") ## Dispersion ratio is 0.78065. Still significant p-value. Best yet though.
testZeroInflation(resbestModel_zeroDisp) ## Ratio is 1.1237. P-value = 0.04. Best yet.
testOutliers(resbestModel_zeroDisp, type = "bootstrap") ## A bit worse. Observed 0.0021 versus expected 0.0073. Significant (p = 0.04)
plot(resbestModel_zeroDisp) ## However, this outlier test is fine. Quantiles look better but some outliers.
check_collinearity(bestModel_zeroDisp) ## VIF marginally better (bivalves = 1.76).
...
## Insignificant p-value. Actual difference between expected (-0.0007) and observed (0.0020) is tiny. Plot looks fine.
...
## Don't worry about convergence warnings, will be for other models.

## Get best model
bestModel <- get.models(combos, delta==0)[[1]]
summary(bestModel)

## Re-define model just to ensure there are no convergence issues.
...
resBestModel <- simulateResiduals(bestModel, re.form = NULL)
testDispersion(resBestModel, type = "DHARMa") ## Ratio is 0.78955. Significant p-value probably down to sample size. However, underdispersion will tend towards conservative results so less of an issue here.
testZeroInflation(resBestModel) ## P = 0.048. Ratio is 1.121.
testOutliers(resBestModel, type = "bootstrap") ## Significant now (p < 2.2e-16). Outliers = 0.00137 observed versus 0.0073 expected
plot(resBestModel) ## "Significant" deviation from dispersion test. Quantiles generally find but outliers.
check_collinearity(bestModel) ## Greatest VIF is bivalve at 1.75. Perfect!
...
## Insignificant (p=0.4794). However, inspecting the plot shows no real correlation in residuals and difference between observed (0.00068) and expected (-0.0007) is negligible.
...
p1
pdf("figures/final/main/genera_2cell_stdOccsWithRepl_marginal_PTME.pdf")
print(p1)
dev.off()
...
p2 <- ggplot(pre, aes(x, y = stdBrach)) +
  ...
  theme(text = element_text(family = "Helvetica"),
        ...
        legend.position = "none")
p2
pdf("figures/final/main/genera_2cell_stdOccsWithRepl_marginal_bivalve_prePTME.pdf")
print(p2)
dev.off()
...
p3 <- ggplot(pre, aes(x, y = stdBrach)) +
  ...
  theme(text = element_text(family = "Helvetica"),
        ...
        legend.position = "none")
p3
pdf("figures/final/main/genera_2cell_stdOccsWithRepl_marginal_bivalve_postPTME.pdf")
print(p3)
dev.off()
...
p4 <- ggplot(prBath, aes(x, y = stdBrach)) +
  ...
  theme(text = element_text(family = "Helvetica"),
        ...
        legend.position = "none")
p4
pdf("figures/final/main/genera_2cell_stdOccsWithRepl_marginal_bathymetry.pdf")
print(p4)
dev.off()
...
p5 <- ggplot(prLith, aes(x, y = stdBrach)) +
  ...
  theme(text = element_text(family = "Helvetica"),
        ...
        legend.position = "none")
p5
pdf("figures/final/main/genera_2cell_stdOccsWithRepl_marginal_lithology.pdf")
print(p5)
dev.off()

#### Plotting coefficients and richness by group ####
## Get clean data for plotting
plotting.data <- read.csv("data/analysis_data/genera_2cell_stdOccs_Repl.csv", row.names = 1)
...
## Define plot title
plot.title <- "Generic richness from standardised occurences\nTwo 50km grid cells per sample"
...
## Export
pdf(file = paste0("figures/final/main/genera_2cell_stdOccsWithRepl_richness.pdf"))
print(scatter)
dev.off()

## Coefficient plot
## Define new title
plot.title2 <- "Model coefficients from standardised occurrences\nTwo 50km grid cells per sample"
...
p7 <- ggplot() +
  ...
  theme(text = element_text(family = "Helvetica"),
        ...
        legend.position = "none")
p7
pdf("figures/final/main/genera_2cell_stdOccsWithRepl_coefficients.pdf")
print(p7)
dev.off()

#### Plotting richness ####
## Read in clean data
richPlotData <- read.csv("data/analysis_data/genera_2cell_stdOccs_Repl.csv", row.names = 1)
...
pdf(file = "figures/final/supplemental/genera_2cell_spatially_standardised_richness_stdOccsWithRepl.pdf")
par(mar = c(6.1, 4.1, 4.1, 2.1))
plot(x = richness$stage_midpoint, y = richness$bivalve_mean, main = "Spatially standardised generic richness (mean +/- 1 SD)\nStandardised occurrences\nTwo 50km grid cells per sample", axes = FALSE, xlim = c(520, 0), ylim = c(0,40), yaxs="i", xaxs="i",
...
lines(x = richness$stage_midpoint, y = richness$brachiopod_mean, xlim = c(520, 0), ylim = c(0,40),
