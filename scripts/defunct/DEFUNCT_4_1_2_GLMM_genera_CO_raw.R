## 4.1.2 GLMM - genera, original covariates, raw
...
test.model.assumptions(model2)
# Dispersion: ratio 1.0006, p < 2.2e-16, significant.
# ZI: ratio 1.8556, p < 2.2e-16, significant.
# Outliers: expected 0.0065, observed 0.14, p < 2.2e-16, significant.
...
# Dispersion: ratio 0.057559, p = 0.584, insignificant.
# ZI: ratio 1.6098, p < 2.2e-16, significant.
# Outliers: expected 0.0067, observed 0.0035, p = 0.58, significant.
# Normality: p = 0.01665, significant.
# Collinearity: all VIFs below 2.5 except for bivalve (3.77) and bivalve:PTME (3.54).
...
# Dispersion: ratio 0.0061, p = 0.776, insignificant.
# ZI: ratio 1.0631, p = 0.48, insignificant.
# Outliers: expected 0.0069, observed 0.0021, p = 0.58, insignificant.
# Normality: p = 0.00026, significant.
# Collinearity: all VIFs below 2.5 except for bivalve (2,63) and bivalve:PTME (2.52).
...
# Normality: p = 0.01749, significant.
# Collinearity: all VIFs below 2.5 except for bivalve (2.91) and bivalve:PTME (2.63).
...
# Dispersion: ratio 3.9962e-0.5, p = 0.904, insignificant.
# ZI: ratio 1.4378, p < 2.2e-16, significant.
# Outliers: expected 0.0021, observed 0.00617, p = 0.28, insignificant.
# Normality: p = 4e-05, significant.
# Collinearity: all VIFs below 2.5 except for bivalve (2.82) and bivalve:PTME (2.7).
...
# Dispersion: ratio 0.00037052, p = 0.88, insignificant
# ZI: ratio 1.293, p = 0.18, insignificant.
# Outliers: expected 0.0065, observed 0.0007, p = 0.02, significant.
# Normality: p = 0.00025, significant.
# Collinearity: all VIFs below 2.5 except for bivalve (2.64) and bivalve:PTME (2.52).
...
bestModel <- glmmTMB(brachiopod ~ bivalve * PTME + bath + reef + lith + AbsLat + n + (bivalve|stage), data = raw_data, family = nbinom1(link = "log"))
...
bestModel_sqrt <- glmmTMB(brachiopod ~ bivalve * PTME + bath + reef + lith + AbsLat + n + (bivalve|stage), data = raw_data, family = nbinom1(link = "sqrt"))
test.model.assumptions(bestModel_sqrt)
# Dispersion: ratio 0.03985, p < 2.2e-16, significant. Worse.
# ZI: ratio 0.85038,  p = 0.152, insignificant. Worse.
# Outliers: expected 0.0058, observed 0, p < 2.2e-16, significant. Worse.
# Normality: p = 0.00328, significant. A bit better
# Collinearity: All VIFs below 2.5.

## Best model has minor deviations in normality. Let's see if Dredge can improve things.

#### Testing for spatial autocorrelation using DHARMa ####
test.spatial.autocorrelation(bestModel, raw_data)
## Expected -0.0008, observed = 0.0067, p = 0.001241, significant. However, no visible correlation from plot.
...
model5 <- glmmTMB(brachiopod ~ bivalve * PTME + bath + reef + lith + AbsLat + n + (bivalve|stage), data = raw_data, family = nbinom1(link = "log"), na.action = "na.fail")
...
## Export
saveRDS(combos, file = "data/sensitivity_testing/genera_CO_raw_dredge_results.Rds")
...
# Dispersion: ratio 0.0045124, p = 0.944, insignificant.
# ZI: ratio 1.0963,  p = 0.32, insignificant.
# Outliers: expected 0.00684, observed 0.00142, p = 0.08, insignificant.
# Normality: p = 0.00024, significant.
# Collinearity: All VIFs below 2.5.
test.spatial.autocorrelation(bestModel_1, raw_data)
## Expected -0.00078, observed = 0.00697, p = 0.0008085, significant. However, no visible correlation from plot.
...
# Dispersion: ratio 5.0766, p = 0.848, insignificant.
# ZI: ratio 1.0815,  p = 0.384, insignificant.
# Outliers: expected 0.00768, observed 0.0007, p = 0.02, significant.
# Normality: p = 0.00016, significant.
# Collinearity: All VIFs below 2.5.
test.spatial.autocorrelation(bestModel_2, raw_data)
## Expected -0.0007, observed = 0.0052, p = 0.008838, significant. However, no visible correlation from plot.
...
# Dispersion: ratio 0.0026627, p = 0.904, insignificant.
# ZI: ratio 1.0925,  p = 0.312, insignificant.
# Outliers: expected 0.0071, observed 0.0021, p = 0.12, insignificant.
# Normality: p = 9e-05, insignificant.
# Collinearity: All VIFs below 2.5
test.spatial.autocorrelation(bestModel_3, raw_data)
## Expected -0.0007, observed = 0.0056341, p = 0.005618, significant. However, no visible correlation from plot.

#### Plotting spatially standardised richness ####
## Read in clean data
richPlotData <- read.csv("data/analysis_data/genera_CO_raw.csv", row.names = 1)
...
pdf(file = "figures/final/supplemental/genera_CO_raw_spatially_standardised_richness.pdf")
par(mar = c(6.1, 4.1, 4.1, 2.1))
plot(x = richness$midpoint, y = richness$bivalve_mean, main = "Spatially standardised generic richness (mean +/- 1 SD)\nRaw richness\n 100km grid cells, original covariate data only", axes = FALSE, xlim = c(520, 0), ylim = c(0,105), yaxs="i", xaxs="i",
...
plotting.data <- read.csv("data/analysis_data/genera_CO_raw.csv", row.names = 1)
...
## Export
pdf(file = paste0("figures/final/main/genera_CO_raw_richness.pdf"))
print(scatter)
dev.off()

#### Plotting coefficients for top models ####
## Define new title
plot.title2 <- "Model coefficients from 100km grid cells\nRaw richness, original covariate data only"
...
## Export
pdf("figures/final/main/genera_CO_raw_coefficients.pdf", width = 13)
print(output)
dev.off()
