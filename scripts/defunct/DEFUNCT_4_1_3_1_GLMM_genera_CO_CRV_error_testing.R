## 4.1.3.1 GLMM - genera, original covariates, classical rarefaction (variable) - type 1 and type 2 error testing
...
## Export results
saveRDS(type2_results, "data/sensitivity_testing/genera_CO_CRV_power_analyses_raw.Rds")
write.csv(type2_out, "data/sensitivity_testing/genera_CO_CRV_power_analyses_power.csv")

#### Shuffling occurrences and richness values for type 1 error testing ####
## Re-calculating rarefaction curves for each cell after shuffling means most, if not all, will not achieve slopes specified.
## Going to use original RC curve values instead
...
## Get RC curve numbers
## Reload curves
gen_RareC <- readRDS("data/sensitivity_testing/master_100_2_2_min3gen_min20occs_RCs.Rds")[,-1]
RC_GCs <- colnames(gen_RareC)

## Remove cells with interpolated covariates
gen_CO_RareC <- gen_RareC[,which(RC_GCs %in% unique(occs$stage_cell))]

## Test asymptotes of cells present to make sure all have some.
gen_CO_RareC_bool <- test_RC_tail_asymptote(RCs = gen_CO_RareC, n = 5, threshold = 0.25)
...
#covariate_values = c("carbonate", "deep", "reef")
#CR = T
#CR_nOccs = gen_CO_n
#fix_stages = F
#n.cores = 8
...
shuffled_occs_between_stages <- shuffle_occurrences(data = occs, reps = 100, stage = "stage", cell = "stage_cell", cell_abun = ref,
                                                    occ_covariates = c("lith_category","bath_category","reef_category"),
                                                    cell_covariates = c("cellLith", "cellBath", "cellReef"),
                                                    covariate_values = c("carbonate", "deep", "reef"), CR = T, CR_nOccs = gen_CO_n,
                                                    fix_stages = F, n_cores = 8)


## Export
saveRDS(shuffled_occs_between_stages, file = "data/sensitivity_testing/genera_CO_CRV_shuffled_occurrences_btwnStgs.Rds")
...
shuffled_occs_within_stages <- shuffle_occurrences(occs, 100, "stage", "stage_cell", cell_abun = ref,
                                                   ...
                                                   cell_covariates = c("cellLith", "cellBath", "cellReef"),
                                                   covariate_values = c("carbonate", "deep", "reef"), CR = T, CR_nOccs = gen_CO_n,
                                                   fix_stages = T, n_cores = 8)
saveRDS(shuffled_occs_within_stages, file = "data/sensitivity_testing/genera_CO_CRV_shuffled_occurrences_wthnStgs.Rds")
...
## Reload raw data with n column, once export
raw_data <- read.csv("data/analysis_data/genera_CO_CRV.csv", header = T, row.names = 1)
...
## Re-export
saveRDS(betweenStages, file = "data/sensitivity_testing/genera_CO_CRV_shuffled_occurrences_btwnStgs.Rds")
saveRDS(withinStages, file = "data/sensitivity_testing/genera_CO_CRV_shuffled_occurrences_wthnStgs.Rds")

#### Type 1 error testing ####
## Read in standardised data
betweenStages <- readRDS("data/sensitivity_testing/genera_CO_CRV_shuffled_occurrences_btwnStgs.Rds")
withinStages <- readRDS("data/sensitivity_testing/genera_CO_CRV_shuffled_occurrences_wthnStgs.Rds")

## Run models
betweenStages_bestModel_1 <- mclapply(1:length(betweenStages), mc.cores = 8, function(y){
  model <- tryCatch(
    {glmer(brachiopod ~ bivalve + PTME + bath + reef + lith + AbsLat + n + (bivalve|stage),
             data = betweenStages[[y]], family = poisson(link = "log"), control = glmerControl(optimizer="bobyqa"))},
    error = function(msg){
      return(NA)
    })
  if(!all(is.na(model))){
...
withinStages_bestModel_1 <- mclapply(1:length(withinStages), mc.cores = 8, function(y){
  model <- tryCatch(
    {glmer(brachiopod ~ bivalve + PTME + bath + reef + lith + AbsLat + n + (bivalve|stage),
           data = withinStages[[y]], family = poisson(link = "log"), control = glmerControl(optimizer="bobyqa"))},
    error = function(msg){
      return(NA)
    })
  if(!all(is.na(model))){
...
betweenStages_bestModel_3 <- mclapply(1:length(betweenStages), mc.cores = 8, function(y){
  model <- tryCatch(
    {glmer(brachiopod ~ bivalve + PTME + bath + lith + AbsLat + n + (bivalve|stage),
           data = betweenStages[[y]], family = poisson(link = "log"), control = glmerControl(optimizer="bobyqa"))},
    error = function(msg){
      return(NA)
    })
...
## Define plotlimits
plot.limits <- list("bivalve" = c(-1,1),
...
coeffs <- c("bivalve", "bath", "lith", "reef", "PTMEPostPTME", "n", "AbsLat")
figure.name <- "genera_CO_CRV_richness_bestModel1_withinStgs_coeffs"
data.name <- "genera_CO_CRV_richness_bestModel1_withinStgs_coeffs"
...
coeffs <- c("bivalve", "bath", "lith", "reef", "PTMEPostPTME", "n", "AbsLat")
figure.name <- "genera_CO_CRV_richness_bestModel1_betweenStgs_coeffs"
data.name <- "genera_CO_CRV_richness_bestModel1_betweenStgs_coeffs"
...
coeffs <- c("bivalve", "bath", "lith", "reef", "PTMEPostPTME", "n", "AbsLat", "bivalve:PTMEPostPTME")
figure.name <- "genera_CO_CRV_richness_bestModel2_withinStgs_coeffs"
data.name <- "genera_CO_CRV_richness_bestModel2_withinStgs_coeffs"
...
coeffs <- c("bivalve", "bath", "lith", "reef", "PTMEPostPTME", "n", "AbsLat", "bivalve:PTMEPostPTME")
figure.name <- "genera_CO_CRV_richness_bestModel2_betweenStgs_coeffs"
data.name <- "genera_CO_CRV_richness_bestModel2_betweenStgs_coeffs"
...
coeffs <- c("bivalve", "bath", "lith", "PTMEPostPTME", "n", "AbsLat")
figure.name <- "genera_CO_CRV_richness_bestModel3_withinStgs_coeffs"
data.name <- "genera_CO_CRV_richness_bestModel3_withinStgs_coeffs"
...
coeffs <- c("bivalve", "bath", "lith", "PTMEPostPTME", "n", "AbsLat")
figure.name <- "genera_CO_CRV_richness_bestModel3_betweenStgs_coeffs"
data.name <- "genera_CO_CRV_richness_bestModel3_betweenStgs_coeffs"
