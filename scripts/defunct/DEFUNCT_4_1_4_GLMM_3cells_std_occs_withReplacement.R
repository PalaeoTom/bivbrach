## 4.1.4 GLMM (main analyses - 50km grid cells, 100km radius, 3 cells per sample, standardised occurrences)
...
## Load data
raw_data <- read.csv("data/analysis_data/genera_3cell_stdOccs_Repl.csv", header = T, row.names = 1)
...
## This is my best model from previous exploration.
...
testDispersion(resBestModel, type = "DHARMa") ## Significantly underdispersed (ratio = 0.67882)
testZeroInflation(resBestModel) ## Insignificant with a little zero inflation. Ratio is 1.107.
testOutliers(resBestModel, type = "bootstrap") ## Significant (p = 0.04). Outliers = 0.0013 observed versus 0.0076 expected
plot(resBestModel) ## Normality fine but dispersion and outlier failure. Quantiles generally fine. Dispersion failure but not really an issue.
check_collinearity(bestModel) ## Greatest VIF is lithology at 1.54. All low.
...
## Insignificant p-value. Actual difference between expected (-0.001279) and observed (0.001353) is tiny. Plot looks fine.
...
bestModel <- glmmTMB(brachiopod ~ bivalve * PTME + bath + lith + reef + (bivalve|stage), data = modelData, ziformula = ~1, dispformula = ~stage, family = nbinom1(link = "log"))

## Test assumptions once again to be sure!
resBestModel <- simulateResiduals(bestModel, re.form = NULL)
testDispersion(resBestModel, type = "DHARMa") ## Ratio is 0.67152. Significant p-value probably down to sample size. However, underdispersion will tend towards conservative results so less of an issue here.
testZeroInflation(resBestModel) ## P = 0.184. Ratio is 1.1064.
testOutliers(resBestModel, type = "bootstrap") ## P = 0.3. Outliers = 0.0039 observed versus 0.0078 expected
plot(resBestModel) ## "Significant" deviation from dispersion test. Quantiles generally find but outliers.
check_collinearity(bestModel) ## Greatest VIF is bivalve at 1.41. Perfect!
...
## Insignificant (p=0.1097). However, inspecting the plot shows no real correlation in residuals and difference between observed (0.0049) and expected (-0.0013) is negligible.
...
bestModel_raw <- glmmTMB(brachiopod ~ bivalve * PTME + bath + lith + reef + (bivalve|stage), ziformula = ~1, dispformula = ~stage, data = modelData_unstd, family = nbinom1(link = "log"))
...
p1
pdf("figures/final/main/genera_3cell_stdOccsWithRepl_marginal_PTME.pdf")
print(p1)
dev.off()
...
## And plot
pBiv <- ggplot(prBiv, aes(x, y = stdBrach)) +
  geom_ribbon(data=prBiv, aes(ymin=std_lower_ci, ymax=std_upper_ci), alpha=0.8, fill="lightblue") +
  ...
  ylab("Standardised brachiopod generic richness") +
  ggtitle("Bivalve generic richness") +
  theme(text = element_text(family = "Helvetica"),
        title = element_text(size = 12),
        axis.text = element_text(size = 11),
        axis.title = element_text(size = 12),
        ...
        legend.position = "none")
pBiv
pdf("figures/final/main/genera_3cell_stdOccsWithRepl_marginal_allBivalves.pdf")
print(pBiv)
dev.off()
...
postModel <- glmmTMB(brachiopod ~ bivalve + bath + lith + reef + (1|stage), ziformula = ~1, dispformula = ~stage, data = postPTME_data, family = nbinom1(link = "log"))
preModel <- glmmTMB(brachiopod ~ bivalve + bath + lith + reef + (1|stage), ziformula = ~1, dispformula = ~stage, data = prePTME_data, family = nbinom1(link = "log"))
...
p2 <- ggplot(pre, aes(x, y = stdBrach)) +
  ...
  theme(text = element_text(family = "Helvetica"),
        ...
        legend.position = "none")
p2
pdf("figures/final/main/genera_3cell_stdOccsWithRepl_marginal_bivalve_prePTME.pdf")
print(p2)
dev.off()
...
p3 <- ggplot(pre, aes(x, y = stdBrach)) +
  ...
  theme(text = element_text(family = "Helvetica"),
        ...
        legend.position = "none")
p3
pdf("figures/final/main/genera_3cell_stdOccsWithRepl_marginal_bivalve_postPTME.pdf")
print(p3)
dev.off()
...
p4 <- ggplot(prBath, aes(x, y = stdBrach)) +
  ...
  theme(text = element_text(family = "Helvetica"),
        ...
        legend.position = "none")
p4
pdf("figures/final/main/genera_3cell_stdOccsWithRepl_marginal_bathymetry.pdf")
print(p4)
dev.off()
...
p5 <- ggplot(prLith, aes(x, y = stdBrach)) +
  ...
  theme(text = element_text(family = "Helvetica"),
        ...
        legend.position = "none")
p5
pdf("figures/final/main/genera_3cell_stdOccsWithRepl_marginal_lithology.pdf")
print(p5)
dev.off()

#### Plotting coefficients and richness by group ####
## Get clean data for plotting
plotting.data <- read.csv("data/analysis_data/genera_3cell_stdOccs_Repl.csv", row.names = 1)
...
## Re-do model
plotModel <- glmmTMB(brachiopod ~ bivalve, data = plotting.data, ziformula = ~1, dispformula = ~stage, family = nbinom1(link = "log"))
...
## Export
pdf(file = paste0("figures/final/main/genera_3cell_stdOccsWithRepl_richness.pdf"))
print(scatter)
dev.off()
...
## Colour scale
colours <- c("lightblue", "darkgreen", "brown", "purple", "darkblue", "pink")
...
axis.labels <- axis.labels[c(5,6,3,1,4,2)]
colours <- colours[c(5,6,3,1,4,2)]
...
p7 <- ggplot() +
  ...
  theme(text = element_text(family = "Helvetica"),
        ...
        legend.position = "none")
p7
pdf("figures/final/main/genera_3cell_stdOccsWithRepl_coefficients.pdf")
print(p7)
dev.off()

#### Plotting richness ####
## Read in clean data
richPlotData <- read.csv("data/analysis_data/genera_3cell_stdOccs_Repl.csv", row.names = 1)
...
pdf(file = "figures/final/supplemental/genera_3cell_spatially_standardised_richness_stdOccsWithRepl.pdf")
par(mar = c(6.1, 4.1, 4.1, 2.1))
plot(x = richness$stage_midpoint, y = richness$bivalve_mean, main = "Spatially standardised generic richness (mean +/- 1 SD)\nStandardised occurrences\nThree 50km grid cells per sample", axes = FALSE, xlim = c(520, 0), ylim = c(0,40), yaxs="i", xaxs="i",
