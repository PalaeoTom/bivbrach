...
## Result: ratio is 2.5248, significant.
...
## Result: ratio of 0.036681, significant.
...
## Result: ratio of 0.12315, significant but less o.
...
## Result: ratio of 0.01656, significant.
...
## Result: ratio of 0.027727. Significant.
...
## Result: ratio of 0.067949, significant
...
testDispersion(resBestModel, type = "DHARMa") ## Ratio of 0.12315, significant.
testZeroInflation(resBestModel) ## Ratio of 1.0749, insignificant.
testOutliers(resBestModel, type = "bootstrap") ## Expect freq. of 0.00713 versus observed 0.000713. Significant.
plot(resBestModel) ## Generally fine - if I can fix dispersion then we are good!
check_collinearity(bestModel) ## All fine!
...
testDispersion(resbestModel_sqrt, type = "DHARMa") ## Ratio of 0.015309, significant and worse.
testZeroInflation(resbestModel_sqrt) ## Ratio of 0.99876, significant. Insignificant and better.
testOutliers(resbestModel_sqrt, type = "bootstrap") ## Expect freq. of 0.00696 versus observed 0. Significant and worse?
plot(resbestModel_sqrt) ## MUCH worse.
check_collinearity(bestModel_sqrt) ## Low VIFs!

## Link function is best one. Now to try a dispersion model.
...
## Check all assumptions at this point
testDispersion(resbestModel_disp, type = "DHARMa") ## Ratio of 0.27004, insignificant. A little better!
testZeroInflation(resbestModel_disp) ## Ratio of 1.0668, insignificant. A little better!
testOutliers(resbestModel_disp, type = "bootstrap") ## Expect freq. of 0.0068 versus observed 0.0014. Not significant but a little better.
plot(resbestModel_disp) ## All fine!
check_collinearity(bestModel_disp) ## All fine - highest is 2.74.

## Reset best model!
## Can this be improved with a dispersion model?
bestModel <- glmmTMB(brachiopod ~ bivalve * PTME + bath + reef + lith + AbsLat + (bivalve|stage), dispformula = ~stage, data = raw_data, family = nbinom1(link = "log"))
resbestModel <- simulateResiduals(bestModel, re.form = NULL)
...
## p = 0.001251. However, no visible correlation from plot.
...
model5 <- glmmTMB(brachiopod ~ bivalve * PTME + bath + reef + lith + AbsLat + (bivalve|stage), dispformula = ~stage, data = modelData, family = nbinom1(link = "log"), na.action = "na.fail")
...
bestModel <- glmmTMB(brachiopod ~ bivalve * PTME + bath + reef + lith + (bivalve|stage), dispformula = ~stage, data = modelData, family = nbinom1(link = "log"))

## Test assumptions once again to be sure!
resBestModel <- simulateResiduals(bestModel, re.form = NULL)
testDispersion(resBestModel, type = "DHARMa") ## Ratio is 0.11576, significant but only just.
testZeroInflation(resBestModel) ## Ratio is 1.0623, insignificant.
testOutliers(resBestModel, type = "bootstrap") ## Significant. Observed 0.0007 versus 0.0066 expected.
plot(resBestModel) ## Nothing to worry about - dispersion test failure but underdispersed.
...
## Inspection shows no obvious correlation but low p-value shows some. Undoubtedly a problem due to replicates.
