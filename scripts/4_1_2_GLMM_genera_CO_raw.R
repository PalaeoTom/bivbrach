...
test.model.assumptions(model2, mode = "D")
## Result: ratio 3.3521, p = <2.2e-16, significant.
...
test.model.assumptions(model3, mode = "D")
## Result: ratio 0.021144, p <2.2e-16, significant.
...
test.model.assumptions(model4, mode = "D")
## Result: ratio 0.47282, p = 0.056, insignificant.
...
test.model.assumptions(modelMixNB, mode = "D")
## Result: ratio 0.27905, p < 2.2e16, significant.
...
test.model.assumptions(modelcompois, mode = "D")
## Result: ratio 0.61837, p < 2.2e-16, significant.

## Genpois
modelgenpois <- glmmTMB(brachiopod ~ bivalve * PTME + bath + reef + lith + AbsLat + (bivalve|stage), data = raw_data, family = genpois(link = "log"))
test.model.assumptions(modelgenpois, mode = "D")
## Result: ratio 0.52847, p < 2.2e-16, significant
...
# Dispersion: ratio 0.92577, p = 0.136, insignificant.
# ZI: ratio 1.1817, p < 2.2e-16, significant.
# Outliers: expected 0.005, observed 0.009, p = 0.06, insignificant.
# Normality: p = 0.358, insignificant.
# Collinearity: PTME VIF 7.15, bivalve:PTME VIF 7.5, all other VIFs below 5.
...
# Dispersion: ratio 0.92577, p = 0.136, insignificant. A bit better!
# ZI: ratio 1.1939,  p < 2.2e-16, significant. Marginally worse.
# Outliers: expected 0.005, observed 0.006, p = 0.98, insignificant. Better!
# Normality: p = 0.510, insignificant. Better
# Collinearity: All VIFs below 5.

## Reset best model to use square root link function
...
## Expected -0.0008, observed = 0.0050, p = 0.01335, significant. However, no visible correlation from plot.
...
## Export
saveRDS(combos, file = "data/sensitivity_testing/genera_CO_CR20_dredge_results.Rds")
## 4 models within 3 dAIC of 0.

## Get models with delta AIC of 2 or less
bestModel <- get.models(combos, delta<2)

## Four models with delta AIC less than 2
## Explore best models. Test for convergence and assumptions.
summary(bestModel[[1]])
bestModel_1 <- glmer(brachiopod ~ bivalve + PTME + bath + reef + lith + (bivalve|stage), data = modelData, family = poisson(link = "sqrt"), control = glmerControl(optimizer="bobyqa"))
test.model.assumptions(bestModel_1)
# Dispersion: ratio 0.93287, p = 0.2, insignificant.
# ZI: ratio 1.193,  p < 2.2e-16, significant.
# Outliers: expected 0.005, observed 0.006, p = 0.98, insignificant.
# Normality: p = 0.73761, insignificant.
# Collinearity: All VIFs below 2.
test.spatial.autocorrelation(bestModel_1, raw_Data)
## Expected -0.0008, observed = 0.0041, p = 0.03558, significant. However, no visible correlation from plot.

summary(bestModel[[2]])
bestModel_2 <- glmer(brachiopod ~ bivalve + PTME + bath + reef + lith + AbsLat + (bivalve|stage), data = modelData, family = poisson(link = "sqrt"), control = glmerControl(optimizer="bobyqa"))
test.model.assumptions(bestModel_2)
# Dispersion: ratio 0.94, p = 0.264, insignificant.
# ZI: ratio 1.1943,  p < 2.2e-16, significant.
# Outliers: expected 0.0051, observed 0.0064, p = 0.6, insignificant.
# Normality: p = 0.60395, insignificant.
# Collinearity: All VIFs below 2.
test.spatial.autocorrelation(bestModel_2, raw_Data)
## Expected -0.0008, observed = 0.0042, p = 0.03176, significant. However, no visible correlation from plot.

summary(bestModel[[3]])
bestModel_3 <- glmer(brachiopod ~ bivalve * PTME + bath + reef + lith + (bivalve|stage), data = modelData, family = poisson(link = "sqrt"), control = glmerControl(optimizer="bobyqa"))
test.model.assumptions(bestModel_3)
# Dispersion: ratio 0.93813, p = 0.216, insignificant.
# ZI: ratio 1.194,  p < 2.2e-16, significant.
# Outliers: expected 0.0050, observed 0.0085, p = 0.14, insignificant.
# Normality: p = 0.57023, insignificant.
# Collinearity: All VIFs below 2 except for bivalve (3.5) and bivalve:PTME (3.07)
test.spatial.autocorrelation(bestModel_3, raw_data)
## Expected -0.0008, observed = 0.0052, p = 0.0095, significant. However, no visible correlation from plot.

summary(bestModel[[4]])
bestModel_4 <- glmer(brachiopod ~ bivalve * PTME + bath + reef + lith + AbsLat + (bivalve|stage), data = modelData, family = poisson(link = "sqrt"), control = glmerControl(optimizer="bobyqa"))
test.model.assumptions(bestModel_4)
# Dispersion: ratio 0.93795, p = 0.256, insignificant.
# ZI: ratio 1.1939,  p < 2.2e-16, significant.
# Outliers: expected 0.0053, observed 0.0057, p = 1, insignificant.
# Normality: p = 0.50974, insignificant.
# Collinearity: All VIFs below 2 except for bivalve (3.47) and bivalve:PTME (3.05)
test.spatial.autocorrelation(bestModel_4, raw_data)
## Expected -0.0008, observed = 0.0049, p = 0.001335, significant. However, no visible correlation from plot.
...
## Export
pdf("figures/final/main/genera_CO_CR20_coefficients.pdf", width = 13)
print(output)
dev.off()
