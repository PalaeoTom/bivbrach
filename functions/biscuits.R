biscuits <- function(dataMat, xy, r, seeding = NULL, rarefaction = "divvySites", reps = 100, nSites = 3, nOccs = 100,
  ...
  if(rarefaction == "divvySites" || rarefaction == "weightedDivvySites"){
    subsamples <- replicate(repSites, cookie(dataMat, seeds, xy, nSites, allPools, weight, coords, crs, output, divvyRarefaction = T, returnSeeds), simplify = FALSE)
    if(returnSeeds){
    ...
    ## get subsamples
    subsamples <- cookie(dataMat, seeds, xy, nSites, allPools, weight, coords, crs, output, divvyRarefaction = F, returnSeeds)
