test.spatial.autocorrelation <- function(model, data, CRS = "EPSG:8857", res = 100000){
  rPrj <- terra::project(x = terra::rast(), y =  CRS, res = res)
  ...
  groupLocations <- data[,c("cell", "cellLong", "cellLat")]
  ...
  resmodel2 <- DHARMa::recalculateResiduals(resmodel, data$cell, mean)
  ## Now test for spatialautocorrelation
  DHARMa::testSpatialAutocorrelation(resmodel2, x = groupLocations$cellLong, y = groupLocations$cellLat)
}
